/*
 *  $Id: TestPhysicalRowIdManager.java,v 1.8 2006/05/31 22:27:12 thompsonbry Exp $
 *
...
public class TestPhysicalRowIdManager extends TestCase {
    ...
    public void testZeroSizeInsertion() throws Exception{
        RecordFile f = new RecordFile(TestRecordFile.testFileName);
        PageManager pm = new PageManager(f);
        PhysicalRowIdManager physMgr = new PhysicalRowIdManager(f, pm);
        ...
        byte[] data = TestUtil.makeRecord(dataSize, (byte) dataVal);
        Location loc = physMgr.insert( data, 0, data.length );
        assertTrue("check " + dataSize + " byte insertion",
               TestUtil.checkRecord(physMgr.fetch(loc), dataSize, (byte) dataVal));


        dataSize = 0;
        dataVal++;
        // insert a 0 byte record.
        data = TestUtil.makeRecord(dataSize, (byte) dataVal);
        try{
        	loc = physMgr.insert( data, 0, data.length );
        	assertTrue(false);
        } catch (IllegalArgumentException e){
        	// we *should* get to here for this test
        }
